...
const ForAdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchAttendanceData(username);
        console.log(data);
        setAttendanceData(data);
        setMarkedDates(
          datesMatcher(
            datesFromTodayToStartOfMonth(),
            data.attendanceRecordsThisMonth
          )
        );
        setLoading(false);
      } catch (error) {
  ...
  useEffect(() => {
    if (attendanceData) {
      setLeavePercentage(
        (attendanceData.numAbsencesThisMonth / leavesAllowed) * 100
      );
      setAttendancePercentage(
        (attendanceData.numAttendancesThisMonth /
          attendanceData.totalDaysThisMonth) *
          100
      );
    }
  ...
  return (
    <div className="dashboard__details">
      <h1 style={{ fontWeight: "bold", textAlign: "center" , margin: "12px"}}>
        Employee Details of {username}
      </h1>
              ...
              <table>
                <tbody>
                  <tr>
                    ...
                    <td>
                      {attendanceData ? attendanceData.numAbsencesThisMonth : 0}
                    </td>
                  </tr>
                  <tr>
                    <td>Leaves Remaining:</td>
                    <td>
                      {attendanceData ? attendanceData.numLeavesRemaining : 0}
                    </td>
                  </tr>
                  <tr>
                    <td>Leaves Taken:</td>
                    <td>
                      {attendanceData ? attendanceData.numLeavesTaken : 0}
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <table>
                <tbody>
                  <tr>
                    <td>Days of Job:</td>
                    <td>
                      {attendanceData ? attendanceData.totalDaysThisMonth : 0}
                    </td>
                  </tr>
                  <tr>
                    <td>Present:</td>
          ...
          <div className="dashboard__graph">
            <BarChart
              xAxis={[
                {
                  scaleType: "band",
                  data: ["Days of Job", "Present", "Absent", "Leaves Taken"],
                },
              ]}
              series={[
                {
                  data: [
                    attendanceData ? attendanceData.totalDaysThisMonth : 0,
                    attendanceData ? attendanceData.numAttendancesThisMonth : 0,
                    attendanceData ? attendanceData.numAbsencesThisMonth : 0,
                    attendanceData ? attendanceData.numLeavesTaken : 0,
                  ],
                },
      ...
      {!loading && !attendanceData && (
        <div className="error">No attendance data available</div>
      )}
    </div>
  );
};

export default ForAdminDashboard;
